...
```r
p + geom_point(aes(color=thermoregulation), size=.5) #for scatter plots, color is the same as fill in bar plots
```
...
```r
homerange %>% 
  ggplot(aes(x = class, fill = taxon, y = log10.mass, group = taxon)) +
  geom_boxplot()
```
