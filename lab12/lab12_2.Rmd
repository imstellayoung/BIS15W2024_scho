...
```{r}
display.brewer.pal(10,"Blues") #sequential palette
```
```{r}
display.brewer.pal(10,"RdPu") #sequential palette
```
...
```{r}
my_palette <- paletteer_d("ggsci::nrc_npg") #store you palette
```
...
```{r}
my_palette <- paletteer_d("ghibli::SpiritedLight") #store you palette
```
