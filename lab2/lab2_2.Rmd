...
```{r}
menu <- c("avocado", "bread", "rice", "steak")
```
...
```{r}
all_harry_potter_matrix <- cbind(harry_potter_matrix, global, total)
all_harry_potter_matrix
```
```{r}
...
```{r}
total_sum <- colSums(all_harry_potter_matrix)
```

```{r}
all_harry_potter_matrix <- rbind(harry_potter_matrix, total_sum)
all_harry_potter_matrix
```
