...
```{r}
mammals %>%
  mutate_all(tolower) ## to make all lowercase
```

Using the across function we can specify individual columns.
```{r}
mammals %>%  ## to only make order and family lowercase
...
mammals %>% 
  ...
  arrange(newborn) ## new variable where if newborn is equal to -999.00 it will replace it to NA and if it is not -999.00 it will be the original newborn number 
```
