...
fisheries_tidy %>% 
  ...
  group_by(country) %>% 
  summarize(catch_sardines = sum(catch, na.rm = TRUE)) %>% 
...
fisheries_tidy %>% 
  ...
  group_by(country) %>% 
  summarise(cephalod_countries = sum(catch, na.rm = TRUE)) %>% 
  arrange(desc(cephalod_countries))
```
...
fisheries_tidy %>% 
  ...
  filter(common_name == "Caramote prawn") %>% 
  group_by(country) %>% 
  summarise(catch_species = sum(catch, na.rm = TRUE)) %>% 
  arrange(desc(catch_species))
```
